...
func TestGetInterCompoundsCommaHandling(t *testing.T) {
	...
	t.Run("repeated params with comma-bearing name reach SQL intact -> 404", func(t *testing.T) {
		...
		// StoToCompoundsMap lowercases/trims each name before the lookup, so the SQL args
		// are the normalized (lower-case) forms; the comma-bearing name still arrives as a
		// single argument. Empty result set => all compounds "not found" => 404 echoing the
		// original-case input tokens.
	...
	t.Run("legacy single comma-joined value is split -> 404", func(t *testing.T) {
		...
		// A single value is comma-split for backward compatibility. Names are lowercased
		// before the DB lookup, so the SQL args are the normalized forms.
		mock.ExpectQuery("SELECT DISTINCT Name").
	...
	t.Run("documented limitation: lone comma-bearing value IS split -> 404", func(t *testing.T) {
		...
		// "Mirtazapin-0,5-Wasser" sent as ONE value is split into two fragments. Each
		// fragment is lowercased before the DB lookup.
		mock.ExpectQuery("SELECT DISTINCT Name").
