...
// --- ADR / QT / Priscus (compound-based) ------------------------------------------------
// dev added compound-based variants of these endpoints. They resolve the compound name via
// SNA_DB and never 404 (unknown compounds resolve to "unknown"/false), so a 200 is expected.

func TestIntegrationADRsCompound(t *testing.T) {
	db := dialOrSkip(t)
	name := sampleCompoundNames(t, db, 1)[0]

	// application=all keeps every representative formulation so the ADR join runs regardless
	// of the sampled compound's available routes.
	w := doGET(newADRController(db).GetAdrsForCompound, "compound="+url.QueryEscape(name)+"&application=all")
	requireOK(t, w)
}

func TestIntegrationQTCompounds(t *testing.T) {
	db := dialOrSkip(t)
	name := sampleCompoundNames(t, db, 1)[0]

	w := doGET(newQTController(db).GetQTStatusByCompound, "compounds="+url.QueryEscape(name)) // SNA_DB/SZG_DB
	requireOK(t, w)
}

func TestIntegrationPriscusCompounds(t *testing.T) {
	db := dialOrSkip(t)
	name := sampleCompoundNames(t, db, 1)[0]

	w := doGET(newPriscusController(db).GetPriscusStatusByCompound, "compounds="+url.QueryEscape(name)) // SNA_DB/SZG_DB
	requireOK(t, w)
}
...
func TestIntegrationProductSearch(t *testing.T) {
	...
	// GetProductSearch parses `name` from the raw query itself (to preserve literal commas),
	// fuzzy-matches Produktname and joins active compounds. A sampled name may match no
...
func TestIntegrationProductList(t *testing.T) {
	...
	// Page 1 exercises the count query and the large product/compound join. An empty dataset
	// would legitimately 404, so only a 5xx (schema mismatch) is treated as a failure.
	w := doGET(newPZNController(db).GetProductList, "page=1")
	requireNoServerError(t, w)
}
