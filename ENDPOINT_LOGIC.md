...
- Optional `indications=true` includes AMTS CAVE indications. By default, product search does not fetch or return indications.
- `indications` is an array of localized `MIN_C.Name` strings.
- Optional `lang=german|english` controls `Standardhinweise` and indication names. An indication stays German when `MIN_C` has no English name.
- Logic:
  ...
  - If `indications=true`, fetches product-specific `Key_MIV` values from `IND_C` for matching `Key_FAM` values.
  - Deduplicates repeated `Key_MIV` values per product and keeps the order from `IND_C.Rang_FI` and `IND_C.Zaehler`.
  - Resolves concise indication names through `MIN_C`. German uses the preferred name. English uses the first English name and falls back to German.
...
- Input PZNs are validated first.
- Optional `indications=true` includes AMTS CAVE indications. By default, product info does not fetch or return indications.
- `indications` is an array of localized `MIN_C.Name` strings.
- Optional `lang=german|english` validates the requested output language. With `lang=english`, the endpoint uses English `MIN_C` names. Missing English indication names stay German.
- Logic:
  - Resolves product family and product group.
  - If `indications=true`, fetches product-specific `Key_MIV` values from `IND_C`.
  - Deduplicates repeated `Key_MIV` values per product and keeps the order from `IND_C.Rang_FI` and `IND_C.Zaehler`.
  - Resolves concise names through `MIN_C`. German uses the preferred name. English uses the first English name and falls back to German.
- Tables:
  - `PAE_DB`
  - `FAM_DB`
  - `IND_C`
